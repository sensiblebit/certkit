...
- Preserve real pre-bundle AIA incomplete diagnostics in `verify`, reject leaf-only `verify --roots` inputs, resolve convert/export issuer chains with signature-aware issuer selection, and fail closed when certstore validation is ambiguous across same-SKI renewals ([#172])
