...
- Fix `TestResolveAIA_DeduplicatesURLs` and `TestResolveAIA_MaxDepth` missing `AllowPrivateNetworks: true`, causing them to fail in environments where `example.com` DNS resolution is unavailable. ([#165])
...
[#165]: https://github.com/sensiblebit/certkit/pull/165
