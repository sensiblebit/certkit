...
- `bundleplan.go` — `PlanBundleExports()` selects scoped candidates deterministically, preflights replacement protection and required bundles, generates selected artifacts in memory, and returns a manifest. `BundleExportPlan.Write()` applies only validated plans and records created/replaced status.
- `bundleplan_existing.go` — Inspects existing bundle leaves and fingerprints contents to reject stale plans and symlinked output bundles.
...
- `scan_export.go` — Managed bundle CLI: scope, explicit `--write`, `--dry-run`, required bundles, output formats, separate input/output password files, and text/JSON plan rendering.
