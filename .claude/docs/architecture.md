...
- `bundleplan_existing.go` — Inspects existing bundle leaves, recognizes managed manifests case-insensitively to retain bundle and selected CA identities, and fingerprints contents to reject stale plans and symlinked output bundles. Conflicting manifest identities are rejected. Malformed JSON/YAML certificate metadata makes replacement ambiguous even alongside a valid leaf; CSR JSON and Kubernetes YAML remain excluded from leaf inspection. Malformed artifact diagnostics retain the filename even when the directory has zero or multiple identifiable leaves.
