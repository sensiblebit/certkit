...
- `bundleplan.go` — `PlanBundleExports()` selects scoped candidates deterministically, records unselected alternatives, checks keys/expiry/trust before replacement conflicts, enforces required bundles, generates selected artifacts in memory, and returns a manifest. It reserves the manifest and refresh-lock names, including case variants, and rejects bundle-directory collisions using case-insensitive comparison. Existing directory names, unselected configuration aliases, and manifest bundle identities protect scoped refreshes even with force enabled; directory names are rechecked under the write lock. Expired leaves require a separate opt-in and retain trust verification at their issuance time. Internal callers can supply `CustomRoots` with the `custom` trust store. `BundleExportPlan.Write()` applies only validated plans and records created/replaced status.
