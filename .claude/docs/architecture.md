...
- `bundleplan.go` — `PlanBundleExports()` selects scoped candidates deterministically, records unselected alternatives, preflights replacement protection and required bundles, generates selected artifacts in memory, and returns a manifest. Expired leaves require a separate opt-in and retain trust verification at their issuance time. `BundleExportPlan.Write()` applies only validated plans and records created/replaced status.
