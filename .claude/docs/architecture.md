...
- `tree.go` — `tree` subcommand. Renders the full Cobra command graph, including built-in commands plus local and inherited flags accepted by each command.
...
- `public/app.js` — Web UI logic. ES module; imports utilities from `utils.js`. Drives WASM certificate processing UI. Features: drag-and-drop file ingestion, category tabs for leaves/intermediates/roots/keys, filter checkboxes for expired/untrusted/unmatched items, bulk export selection, and click-to-sort on all table columns (default: certs by expiry DESC, keys by matched DESC then type ASC).
