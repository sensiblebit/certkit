...
window.certkitOnAIAComplete = function (warningsJSON) {
    ...
    if (warnings.length > 0) {
        ...
        showStatus(`Chain resolution done (${warnings.length} warning(s))`, false, false);
    } else {
...
selectAll.addEventListener("change", () => {
    const checkboxes = resultsBody.querySelectorAll(".cert-select:not(:disabled)");
...
function updateExportBtn() {
    ...
    exportBtn.textContent = count > 0 ? `Export ${count} Bundle${count !== 1 ? "s" : ""} (ZIP)` : "Export Bundles (ZIP)";
}
